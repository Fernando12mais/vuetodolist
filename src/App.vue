...
<template>
  <main>
    ...
    <section class="w-full">
      <h1>
        {{ tasks.length ? `Tarefas:(${tasks.length})` : "Crie uma tarefa" }}
      </h1>
      <div
        v-if="tasks.length"
        class="max-w-lg mx-auto flex flex-col gap-3 bg-slate-600 p-4 rounded"
      >
        <Button
          variant="delete"
          v-if="tasks.length > 3"
        ...
        >
        <div
          class="bg-slate-800/70 p-3 rounded w-full animate-fadeIn max-h-96 overflow-auto flex flex-col gap-4 relative"
        >
